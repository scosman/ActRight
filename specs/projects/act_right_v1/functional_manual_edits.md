...
The docstring is **approved by the human, respected by the agent**. Heal rewrites the test body. Heal does not rewrite the docstring unless explicitly asked (e.g. intent changed, re-author). See §6.3 for the heal-edit policy.
...
Act's skills live in a git repo. Installing act means cloning that repo into the location the project's agent scans for skills. For Claude Code in v1, that's `.claude/skills/act` at the project root:

```sh
# from the project root
git clone --single-branch --depth 1 https://github.com/<owner>/act.git ~/.claude/skills/act
```
...
8. **Print "next steps"**: how to run tests, how to invoke `/act:new`, and whether the app-server sanity test passed.

**Idempotent**: safe to re-run. Detects what already exists (Playwright install, MCP registration, example tests, `webServer`/`baseURL` config) and skips; never overwrites an existing Playwright config or example tests. Re-running after a skipped step 6 offers to complete it.
...
1. **`/act:setup` dev-server detection heuristics**: the spec says detect `dev`/`start`/`preview` scripts and propose them (§5.2 step 6). Confirm this set is right; settle the fallback behavior when nothing plausible is detected (prompt user for a command vs. skip step 6 entirely).
2. **Bundled-scripts pattern** (§5.7): confirm this is the general mechanism for non-trivial helpers (TS parsing, AST work, project introspection), replacing any act MCP. The starter script list is `list-fixtures.ts`, `list-act-tests.ts`, `get-act-doc.ts`; happy to expand or trim.
3. **Skills package layout / naming** for distribution: any branding/naming we want to lock in at the functional level (e.g. `@actright/skills`), or defer to architecture?
